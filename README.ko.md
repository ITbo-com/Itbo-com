<!-- ⬇⬇⬇ 여기부터 복사해서 GitHub의 README.ko.md(국문)에 붙여넣기 ⬇⬇⬇ -->
<div align="center">
  <a href="README.md">🇺🇸 English</a> &nbsp;·&nbsp; <a href="README.ko.md">🇰🇷 한국어</a>
</div>

<div align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&weight=600&size=22&pause=900&center=true&vCenter=true&width=600&color=58A6FF&lines=%EB%B3%B4%EC%95%88%EC%9D%84+%EC%9D%B4%ED%95%B4%ED%95%98%EA%B3%A0+%EB%A7%8C%EB%93%9C%EB%8A%94+%ED%92%80%EC%8A%A4%ED%83%9D+%EA%B0%9C%EB%B0%9C%EC%9E%90;%EB%B0%B1%EC%97%94%EB%93%9C%C2%B7%EB%B3%B4%EC%95%88+%40+pLAWcess" alt="typing banner" />
</div>

## 👋 안녕하세요, 송인보입니다

> **보안을 이해하고 직접 만드는 풀스택 개발자** · 고려대학교 컴퓨터학과

- 🔐 **백엔드·보안** 개발자 — 시스템이 어떻게 뚫리는지 이해하고, 뚫리지 않게 만드는 데 관심이 있습니다.
- 🧩 웹(Next.js)·앱(Flutter)를 아우르는 풀스택.
- 🏆 로스쿨 멘토링 플랫폼 **pLAWcess**를 백엔드·보안으로 개발 — 교내 창업 지원과 소프트웨어 경진대회 수상.
- 🌱 요즘은 **정보보안**(OWASP·시스템 보안) 심화와 꾸준한 알고리즘 트레이닝.

### 🏅 주요 성과
- 🏆 **NE:XT 소프트웨어 경진대회** — 장려상 (45팀 중 상위 8팀) · 교내 창업 동아리 선정(100만원 지원)
- 🔐 **보안** — pLAWcess에서 OWASP Top 10 기반 점검·보안 리포트 작성
- 📜 자격증 — **리눅스마스터 1급 · 네트워크관리사 2급 · SQLD · 정보처리기능사 · 정보기기운용기능사**
- 🤖 **LG Aimers** — LLM 경량화·양자화 과정 수료

### 🛠 기술 스택
![TypeScript](https://img.shields.io/badge/TypeScript-3178C6?style=flat-square&logo=typescript&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat-square&logo=python&logoColor=white)
![C](https://img.shields.io/badge/C-A8B9CC?style=flat-square&logo=c&logoColor=black)
![Dart](https://img.shields.io/badge/Dart-0175C2?style=flat-square&logo=dart&logoColor=white)

![Next.js](https://img.shields.io/badge/Next.js-000000?style=flat-square&logo=nextdotjs&logoColor=white)
![React](https://img.shields.io/badge/React-087EA4?style=flat-square&logo=react&logoColor=white)
![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat-square&logo=flutter&logoColor=white)
![Node.js](https://img.shields.io/badge/Node.js-5FA04E?style=flat-square&logo=nodedotjs&logoColor=white)

![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat-square&logo=postgresql&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-1A1A1A?style=flat-square&logo=linux&logoColor=white)
![Kali](https://img.shields.io/badge/Kali_Linux-557C94?style=flat-square&logo=kalilinux&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat-square&logo=git&logoColor=white)

### 📊 GitHub
<div align="center">
  <a href="https://github.com/Itbo-com">
    <img height="160" alt="GitHub Stats" src="https://github-readme-stats.vercel.app/api?username=Itbo-com&show_icons=true&count_private=true&include_all_commits=true&hide_border=true&locale=ko&theme=default#gh-light-mode-only" />
    <img height="160" alt="GitHub Stats" src="https://github-readme-stats.vercel.app/api?username=Itbo-com&show_icons=true&count_private=true&include_all_commits=true&hide_border=true&locale=ko&theme=github_dark#gh-dark-mode-only" />
  </a>
  <a href="https://solved.ac/profile/inbo2002">
    <img height="160" alt="solved.ac" src="http://mazassumnida.wtf/api/v2/generate_badge?boj=inbo2002" />
  </a>
</div>

### 🚀 프로젝트
> 레포지토리는 organization private에 있어, 여기서는 역할·성과 중심으로 소개합니다.

| 프로젝트 | 내용 | 스택 | 역할 / 성과 |
|---|---|---|---|
| **pLAWcess** | 로스쿨 멘토링 매칭 플랫폼 | Next.js · PostgreSQL · TS | 백엔드·**보안** · 🏆 NE:XT 장려상, 창업지원 |
| **BigContest 2025** | 가맹점 폐업위기 조기경보 AI | LightGBM · SHAP · Python | 데이터·모델링 (팀) |
| **연고전 AI 해커톤** | 오염문서 RAG + 보안 방어 | Upstage Solar LLM | 참가 |
| **LG Aimers** | LLM 경량화·양자화 | W8A8 · KV-cache | ✅ 수료 |

### 📫 연락
<div align="center">
  <a href="mailto:inbo2002@korea.ac.kr"><img src="https://img.shields.io/badge/Email-EA4335?style=for-the-badge&logo=gmail&logoColor=white" alt="Email" /></a>
  <a href="https://www.linkedin.com/in/송인보-‍-793880255"><img src="https://img.shields.io/badge/LinkedIn-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white" alt="LinkedIn" /></a>
  <a href="https://itbo.tistory.com/"><img src="https://img.shields.io/badge/Blog-000000?style=for-the-badge&logo=tistory&logoColor=white" alt="Blog" /></a>
  <a href="https://solved.ac/profile/inbo2002"><img src="https://img.shields.io/badge/solved.ac-0076C0?style=for-the-badge" alt="solved.ac" /></a>
</div>
<!-- ⬆⬆⬆ 여기까지 복사 ⬆⬆⬆ -->
